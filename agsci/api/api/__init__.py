...
class BaseView(BrowserView):
    ...
    def mapProductType(self, data):
        ...
        education_format_mapping = {
            ...
            'Curriculum Group' : 'Tools and Apps',
        ...
        if plone_product_type:
            ...
            if ICventEvent.providedBy(self.context):
                ...
                # Fix the gunky HTML in 'product_detail/product_description' generated by
                # the new Cvent Flex editor (React?)
                #
                # Also (hopefully temporarily) slightly fudge start times for sessions with
                # the same start time to prevent Magento from using those as a unique key.
                if 'product_detail' in data:
                    product_detail = data['product_detail']

                    if product_detail and isinstance(product_detail, (list, tuple)):

                        all_start_times = []

                        for _ in product_detail:
                            if isinstance(_, dict):
                                ...
                                if 'start_time' in _:
                                    ...
                                    if start_time in all_start_times:
                                        ...
                                        # Convert to a DateTime, and add a second
                                        # for each instance
                                        st_count = all_start_times.count(start_time)
                                        st_adjusted = DateTime(start_time) + st_count/86400.0
                                        _['start_time'] = toISO(st_adjusted)
